...
for i, (yy, name, pat) in enumerate([
    ...
    if yy + 70 < 430:  # only between composite rows -- must not bleed into the Single-structure band below
        c.line(300, yy + 70, 880, yy + 70, w=1.2, color=LIGHT)
    c.text(430, yy + 8, name, size=15, anchor="mr")
    wick_core(440, 840, yy + 8, pat)

# ---------- single row ----------
c.text(430, 497, "Non-woven fabric", size=15, anchor="mr")
c.text(430, 515, "(flat core)", size=13, anchor="mr", color="#555")
wick_core(440, 840, 505, "nonwoven")

# ---------- save ----------
repo = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
c.save(f"{repo}/source/fig-3.15-wick-drain-cross-sections.svg",
       f"{repo}/export/fig-3.15-wick-drain-cross-sections.png")
